...
for i in range(1,5):
    ...
    plt.plot(data[:, 0], filtered/scale_factor, label='Unpruned '+str(i), linewidth=0.75,color=colors[i-1], linestyle='-')
...
for i in range(1,5):_
    ...
    window = int(len(data[:, 1])/30)
    filtered = signal.savgol_filter(data[:, 1],window+1 if window%2 == 0 else window,5)
    plt.plot(data[:, 0], filtered/scale_factor, label='Pruned '+str(i), linewidth=0.75,color=colors[i-1], linestyle='-')
...
plt.xlim(0, 60000)
plt.yticks(ticks=[450,500,550,600,650],labels=['450','500','550','600','650'])
plt.ylim(400, 700)
plt.tight_layout()
plt.legend(loc='upper left', ncol=2, labelspacing=.2, columnspacing=.25, borderpad=.25)
